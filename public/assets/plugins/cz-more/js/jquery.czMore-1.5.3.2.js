    ...
    $.fn.czMore = function (options) {
        ...
        return this.each(function () {
            ...
            btnPlus.css({
                'float': 'right',
                'border': '0px',
                'background-image': 'url("'+base_url+'/public/assets/plugins/cz-more/img/add.png")',
            ...
            if (recordset.length) {
                obj.siblings("#btnPlus").click(function () {
					var counter = 0;
					$(".recordset").find("input[type=text][name!=''].require").each(function(index) {
						if($(this).get(0).nextSibling) $(this).get(0).nextSibling.remove();
						if(this.name == "invite[email][]" && !_functions.validate_email($(this).val()) && $(this).val()) {
							$(this).after("Enter valid email!");
							counter++;
						} else if (!$(this).val()) {
				...
				obj.siblings("#btnPlus").trigger('click');
            }
            ...
            function loadMinus(recordset) {
                ...
                btnMinus.css({
                    'float': 'right',
                    'border': '0px',
                    'background-image': 'url("'+base_url+'/public/assets/plugins/cz-more/img/remove.png")',
                    'background-position': 'center center',
                    'background-repeat': 'no-repeat',
                    'height': '25px',
                    'width': '25px',
